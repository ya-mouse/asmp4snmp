...
netsnmp_udp6_fmtaddr(netsnmp_transport *t, void *data, int len)
{
    ...
    } else if (t != NULL && t->data != NULL && ((struct asmp_connection *)(t->data))->addr_pair != NULL) {
        to = (struct sockaddr_in6 *) ((struct asmp_connection *)t->data)->addr_pair;
    }
    if (to == NULL) {
...
//               t->data_length == sizeof(struct sockaddr_in6)) {
        to = (struct sockaddr *) ((struct asmp_connection *)t->data)->addr_pair;
    }
...
netsnmp_udp6_transport(struct sockaddr_in6 *addr, int local)
{
    ...
    t->domain_length =
        sizeof(netsnmp_asmpAIDPDomain) / sizeof(oid);
    ...
    if (local) {
	  ...
	  if (setsockopt(t->sock, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&one, sizeof(one)) != 0) {
        ...
        asmp->addr_pair = malloc(sizeof(struct sockaddr_in6));
        memcpy(asmp->addr_pair, addr, sizeof(struct sockaddr_in6));
...
netsnmp_udp6_ctor(void)
{
    ...
    asmpDomain.f_create_from_tstring_new = netsnmp_udp6_create_tstring;
    asmpDomain.f_create_from_ostring = netsnmp_udp6_create_ostring;
    asmpDomain.prefix = (const char**)calloc(1, sizeof(char *));
